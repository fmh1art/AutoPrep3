...
def query_and_cache(req: QueryAndCacheRequest):
    result = server.query_and_cache(req.input_messages, req.response_msg)
    return CachedTokenResponse(**result)


@app.post("/compute", response_model=CachedTokenResponse)
def compute(req: ComputeRequest):
    result = server.compute_cached_tokens(req.input_messages)
    return CachedTokenResponse(**result)


@app.post("/cache", response_model=CachedTokenResponse)
def cache(req: CacheRequest):
    result = server.cache_conversation(req.input_messages, req.response_msg)
    return CachedTokenResponse(**result)
