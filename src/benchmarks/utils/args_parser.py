...
from src.benchmarks.utils.critics import add_critic_args
...
def add_prompt_path_argument(parser: argparse.ArgumentParser, caller_file: str) -> None:
    ...
    Users can pass a bare filename (e.g. ``query.j2``), which is resolved
    ...
    assert (prompt_dir / "query.j2").exists(), (
        f"Default prompt {prompt_dir / 'query.j2'} not found"
    )

    def _resolve_prompt(value: str) -> str:
        """Resolve a filename or path to an absolute prompt template path."""
        # Accept bare filenames (e.g. "query.j2") and resolve them.
    ...
    parser.add_argument(
        ...
        default=str(prompt_dir / "query.j2"),
        metavar="{" + ",".join(templates) + "}",
        help="Prompt template filename (default: query.j2)",
    )
