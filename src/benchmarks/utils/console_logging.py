...
def print_trajectory_line(
    ...
    - 若启用富文本日志（RICH_LOGGING=1/true/yes），同时打印到控制台（ANSI）。
    - 无论是否启用，都尝试写入名为 "trajectory" 的 logger（若已配置 handler）。
    """
    try:
        ...
        if not line:
            return
        # 文件日志（如果外部已配置 'trajectory' logger 的 FileHandler）
        try:
            import logging

            traj_logger = logging.getLogger("trajectory")
            if traj_logger.handlers:
                traj_logger.info(line)
        except Exception:
            pass
        # 控制台打印（富文本模式）
        if _rich_logging_enabled() and sys.__stdout__ is not None:
