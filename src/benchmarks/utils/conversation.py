...
from src.benchmarks.utils.console_logging import print_trajectory_line
    ...
    def _persist_event(event: Event) -> None:
        ...
        try:
            ...
            if event_size <= MAX_EVENT_SIZE_BYTES:
                ...
                # 将完整事件序列化内容作为消息写入日志，便于文件中查看完整轨迹
                logger.info("[CONV_EVENT] %s", serialized)
            else:
                # Large event: log metadata only
                metadata = _extract_event_metadata(event)
                logger.info("[CONV_EVENT_META] size=%d truncated=1 meta=%s", event_size, metadata)
