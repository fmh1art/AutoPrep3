...
class ExecuteReadFullObservation(Tool):

    def tool_definition(self):
        return {
            "type": "function",
            "function": {
                "name": "ReadFullObservation",
                "description": (
                    "Read the full (un-summarized) observation for specific previous steps.\n"
                    "Use this when the summarized observation is insufficient and you need "
                    "the complete details of certain steps to proceed.\n"
                    "The step indices are 1-based, matching the [Step N] tags in observations."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "target_step_indices": {
                            ...
                            "items": {"type": "integer"},
                            "description": (
                                "List of 1-based step indices whose full observations you want to read. "
                                "For example, [1, 3, 5] means read the full observation of step 1, 3, and 5."
                            ),
                        },
                    },
                    "required": ["target_step_indices"],
                },
            }
        }
    ...
    def execute(self, args: dict, workspace: DockerWorkspace) -> str:
        raise NotImplementedError(
            "ReadFullObservation execution is handled by CustomizedCodeAgent"
        )
...
class ExecuteCreateSubagent_MetaAgentWithoutTools(Tool):
    ...
    def tool_definition(self):
        return {
            ...
            "function": {
                "name": "CreateSubagent",
                "description": (
                    "Create a sub-agent to execute a subtask. The sub-agent will "
                    "receive the original task description, and trajectories from related prior "
                    "subtasks for the current subtask."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "subtask": {
                            "type": "string",
                            "description": (
                                "Brief but comprehensive description of what this sub-agent should accomplish. Make sure you subtask is specific."
                            ),
                        },
                        "related_subtask_index": {
                            ...
                            "description": (
                                ...
                                "1-based subtask numbers shown in the completed subtasks list. "
                            ),
                        },
                    },
                    "required": ["subtask", "related_subtask_index"],
                },
            }
        }
    ...
    def execute(self, args: dict, workspace: DockerWorkspace) -> str:
        raise NotImplementedError(
            "CreateSubagent execution is handled by CustomizedMetaAgent.observe()"
        )


class ExecuteCreateSubagent_MetaAgentWithTools(Tool):
    ...
    def tool_definition(self):
        return {
            ...
            "function": {
                ...
                "description": (
                    "Create a sub-agent to execute a subtask. The sub-agent will "
                    "receive the original task description, and trajectory consisting of related prior "
                    "steps."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "subtask": {
                            "type": "string",
                            "description": (
                                "Comprehensive description of what this sub-agent should accomplish. "
                                "The subtask should be specific with comprehensive information."
                            ),
                        },
                        "related_step_indexes": {
                            "type": "array",
                            "items": {"type": "integer"},
                            "description": (
                                "Indices of previously completed steps whose "
                                "trajectories are relevant to this subtask. These are the "
                                "1-based step numbers shown in the completed steps list. "
                            ),
                        },
                    },
                    "required": ["subtask", "related_step_indexes"],
                },
            }
        }
...
class ExecuteSubtaskFinish(Tool):
        ...
        return {
            ...
            "function": {
                ...
                "description": (
                    ...
                    "\n"
                    "You MUST provide the `useful_trajectory_indexes` parameter as a structured "
                    "argument (not embedded in text):\n"
                    "  - Step indexes are 1-based — each observation ends with [Step N] where N "
                    "starts from 1. Use those N values as your indexes.\n"
                    ...
                    "  - EXCLUDE: exploration that didn't lead anywhere, repeated reads, dead-end attempts.\n"
                    "  - Including everything is WORSE than including nothing.\n"
                    "  - If you genuinely had no useful steps, pass [].\n"
                    "\n"
                ...
                "parameters": {
                    ...
                    "properties": {
                        "useful_trajectory_indexes": {
                            ...
                            "items": {"type": "integer"},
                            "description": (
                                "REQUIRED. The 1-based indexes of trajectory steps that the next "
                                "sub-agent MUST see. Match the N in [Step N] from observations. "
                                "Exclude exploratory/dead-end steps."
                            ),
                        },
                        "message": {
                            "type": "string",
                            "description": "Brief summary of what this sub-agent accomplished in 1~2 sentence.",
                        },
                    },
                    "required": ["useful_trajectory_indexes"],
                },
            }
        }

    def execute(self, args: dict, workspace: DockerWorkspace) -> str:
        useful_indexes = args.get("useful_trajectory_indexes", [])
        message = args.get("message", "")
        parts = []
        if message:
            parts.append(message)
        parts.append(f"Useful trajectory indexes: {useful_indexes}")
        return "\n".join(parts)


class ExecuteInstantiateMetaTask(Tool):

    def tool_definition(self):
        return {
            "type": "function",
            "function": {
                "name": "instantiate_meta_task",
                "description": (
                    "Submit the instantiated meta task result and finish the task.\n"
                    "\n"
                    "Call this tool when you have explored the repository and created a concrete "
                    "problem statement for the meta task. The problem_statement must reference "
                    "actual files, functions, or classes from the repository and read like a real "
                    "GitHub issue.\n"
                    "\n"
                    "This tool also signals task completion — you will not be able to make further "
                    "tool calls after this."
                ),
                "parameters": {
                    "type": "object",
                    "properties": {
                        "problem_statement": {
                            "type": "string",
                            "description": (
                                "A detailed, concrete description of the task to be solved. "
                                "Must include specific file paths, function names, or class names "
                                "from the repository. Should read like a real GitHub issue with "
    ...
    def execute(self, args: dict, workspace: DockerWorkspace) -> str:
        ...
        return f"Meta task instantiated successfully."
