"""
并行运行SWE-bench评估 — MetaAgent (Plan + Code Agent Baseline) 模式

简化版基线框架：
  - PlanAgent：使用 CreateSubagent + terminate 分解任务
  - SubAgent：封装 CodeAgent（bash, file_editor, finish）执行子任务
  - 无 Skill 进化、无成本检测、无运行时检测
"""
...
def run_single_instance_meta_agent(args_dict):
    ...
    with instance_context(log_dir, instance_id):
        logger.info(f"[Worker-MetaAgent] Starting {instance_id}")
        ...
        try:
            ...
            logger.info(f"[Worker-MetaAgent] Cloning {repo_url} @ {base_commit} into {repo_path}")
            ...
            logger.info(f"[Worker-MetaAgent] Repository cloned successfully for {instance_id}")

            task_description = str(instance.get("problem_statement", "")).strip()

            from src.agent.meta_agent import MetaAgentPipeline

            pipeline = MetaAgentPipeline(**pipeline_config)
            ...
            logger.info(
                f"[Worker-MetaAgent] Completed {instance_id}: resolved={result['resolved']}, "
                f"subtasks={result['subtask_count']}"
            )
            return result

        except Exception as e:
            logger.error(f"[Worker-MetaAgent] Error in {instance_id}: {e}")
            traceback.print_exc()
            return {
                "instance_id": instance_id,
                "error": str(e),
            ...
            if workspace is not None:
                try:
                    ...
                    logger.warning(f"[Worker-MetaAgent] Cleanup failed for {instance_id}: {cleanup_err}")


def main():
    parser = argparse.ArgumentParser(
        description="并行运行SWE-bench评估 — MetaAgent (Plan + Code Agent Baseline) 模式"
    )
    ...
    parser.add_argument("--cheap-config", type=str, required=True, help="便宜模型配置文件（本框架中与 exp-config 使用同一配置）")
    ...
    if args.output_dir is None:
        ...
        args.output_dir = (
            f"./_tmp/meta_agent_limit{args.eval_limit}"
            f"_{exp_name}"
            f"_{timestamp}"
        )
    ...
    if args.parallel <= 1:
        for task in tasks:
            result = run_single_instance_meta_agent(task)
        ...
        with ProcessPoolExecutor(max_workers=args.parallel) as executor:
            futures = {
                executor.submit(run_single_instance_meta_agent, task): task
                for task in tasks
            }
