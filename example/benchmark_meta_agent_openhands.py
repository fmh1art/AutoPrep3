"""
并行运行SWE-bench评估 — MetaAgentOpenHands (Plan + Code Agent via OpenHands SDK) 模式

基于 OpenHands SDK 的 Meta Agent Pipeline：
  - MetaPlanAgentOpenHands：使用 SDK Agent + Conversation 的规划 Agent
  - MetaSubAgentOpenHands：使用 SDK Agent + Conversation 的代码执行 Agent
  - MetaAgentPipelineOpenHands：编排 MetaPlanAgent → MetaSubAgent 循环
"""
...
def run_single_instance_meta_agent_openhands(args_dict):
    ...
    with instance_context(log_dir, instance_id):
        logger.info(f"[Worker-MetaAgentOH] Starting {instance_id}")
        ...
        try:
            ...
            logger.info(f"[Worker-MetaAgentOH] Cloning {repo_url} @ {base_commit} into {repo_path}")
            ...
            logger.info(f"[Worker-MetaAgentOH] Repository cloned successfully for {instance_id}")

            task_description = str(instance.get("problem_statement", "")).strip()

            from openhands.sdk import LLM
            from src.agent.meta_agent_openhands import MetaAgentPipelineOpenHands

            llm = LLM(
                model=f"openai/{pipeline_config['llm_name']}",
                api_key=pipeline_config["llm_key"],
                base_url=pipeline_config.get("llm_base_url"),
            )

            pipeline = MetaAgentPipelineOpenHands(
                llm=llm,
                max_steps_per_subagent=pipeline_config["max_steps_per_subagent"],
                max_planning_steps=pipeline_config["max_planning_steps"],
                max_planning_total_steps=pipeline_config["max_planning_total_steps"],
                selective_fallback_rule=pipeline_config["selective_fallback_rule"],
                bash_observation_threshold=pipeline_config["bash_observation_threshold"],
                subagent_observation_max_length=pipeline_config["subagent_observation_max_length"],
            )
            ...
            pipeline_result = pipeline.run(
                ...
                workspace=workspace,
                repo_path=repo_path,
                output_dir=os.path.join(tmp_root, "log", instance_id),
            )
            ...
            logger.info(
                f"[Worker-MetaAgentOH] Completed {instance_id}: resolved={result['resolved']}, "
                f"subtasks={result['subtask_count']}"
            )
            return result

        except Exception as e:
            logger.error(f"[Worker-MetaAgentOH] Error in {instance_id}: {e}")
            traceback.print_exc()
            return {
                "instance_id": instance_id,
                "error": str(e),
            ...
            if workspace is not None:
                try:
                    ...
                    logger.warning(f"[Worker-MetaAgentOH] Cleanup failed for {instance_id}: {cleanup_err}")


def main():
    parser = argparse.ArgumentParser(
        description="并行运行SWE-bench评估 — MetaAgentOpenHands (Plan + Code Agent via OpenHands SDK) 模式"
    )
    ...
    if args.output_dir is None:
        ...
        args.output_dir = (
            f"./_tmp/meta_agent_openhands_limit{args.eval_limit}"
            f"_{exp_name}"
            f"_{timestamp}"
        )
    ...
    pipeline_config = {
        "llm_name": exp_cfg.get("llm_name", "gpt-4"),
        "llm_key": exp_cfg.get("key", ""),
        "llm_base_url": exp_cfg.get("openai_base_url"),
        "max_steps_per_subagent": args.max_steps_per_subagent,
        "max_planning_steps": args.max_planning_steps,
        "max_planning_total_steps": args.max_planning_total_steps,
        "selective_fallback_rule": args.selective_fallback_rule,
    ...
    if args.parallel <= 1:
        for task in tasks:
            result = run_single_instance_meta_agent_openhands(task)
        ...
        with ProcessPoolExecutor(max_workers=args.parallel) as executor:
            futures = {
                executor.submit(run_single_instance_meta_agent_openhands, task): task
                for task in tasks
            }
