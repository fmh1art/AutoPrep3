"""
并行运行SWE-bench评估 — COAT V2 (Self-Evolving Dual-LLM PlanAgent) 模式
...
def _log_eval_progress(result: dict, results_so_far: list[dict], total_instances: int) -> None:
    """实时打印单实例评估结果 + 当前累计 resolved rate（COAT V2：含 skill evolution 统计）。"""
...
# ---------------------------------------------------------------------------
# COAT V2 PlanAgent worker
# ---------------------------------------------------------------------------

def run_single_instance_plan_agent_v2(args_dict):
    ...
    with instance_context(log_dir, instance_id):
        logger.info(f"[Worker-COAT-V2] Starting {instance_id}")
        ...
        try:
            ...
            logger.info(f"[Worker-COAT-V2] Cloning {repo_url} @ {base_commit} into {repo_path}")
            ...
            logger.info(f"[Worker-COAT-V2] Repository cloned successfully for {instance_id}")

            task_description = str(instance.get("problem_statement", "")).strip()

            from src.agent.plan_agent_v2 import PlanAgentPipelineV2

            pipeline = PlanAgentPipelineV2(**pipeline_config)
            ...
            logger.info(
                f"[Worker-COAT-V2] Completed {instance_id}: resolved={result['resolved']}, "
            ...
            logger.error(f"[Worker-COAT-V2] Error in {instance_id}: {e}")
            traceback.print_exc()
            return {
                "instance_id": instance_id,
                "error": str(e),
            ...
            if workspace is not None:
                try:
                    ...
                    logger.warning(f"[Worker-COAT-V2] Cleanup failed for {instance_id}: {cleanup_err}")
...
def main():
    parser = argparse.ArgumentParser(
        description="并行运行SWE-bench评估 — COAT V2 (Self-Evolving Dual-LLM PlanAgent) 模式"
    )
    ...
    if args.output_dir is None:
        ...
        args.output_dir = (
            f"./_tmp/coat_v2_{args.llm_selection_strategy}"
    ...
    if args.parallel <= 1:
        for task in tasks:
            result = run_single_instance_plan_agent_v2(task)
        ...
        with ProcessPoolExecutor(max_workers=args.parallel) as executor:
            futures = {
                executor.submit(run_single_instance_plan_agent_v2, task): task
                for task in tasks
            }
