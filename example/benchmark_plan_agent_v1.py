"""
并行运行SWE-bench评估 — COAT V1 (Dual-LLM PlanAgent) 模式
...
def _log_eval_progress(result: dict, results_so_far: list[dict], total_instances: int) -> None:
    """实时打印单实例评估结果 + 当前累计 resolved rate（COAT V1：含 cheap/expensive 统计）。"""
    ...
    if not has_error:
        sc = result.get("subtask_count", 0)
        cs = result.get("cheap_subtask_count", 0)
        es = result.get("expensive_subtask_count", 0)
        parts.append(f"subtasks={sc}(cheap={cs},exp={es})")
    if total_tokens:
...
# ---------------------------------------------------------------------------
# COAT V1 PlanAgent worker
# ---------------------------------------------------------------------------

def run_single_instance_plan_agent_v1(args_dict):
    ...
    with instance_context(log_dir, instance_id):
        logger.info(f"[Worker-COAT-V1] Starting {instance_id}")
        ...
        try:
            ...
            logger.info(f"[Worker-COAT-V1] Cloning {repo_url} @ {base_commit} into {repo_path}")
            ...
            logger.info(f"[Worker-COAT-V1] Repository cloned successfully for {instance_id}")

            task_description = str(instance.get("problem_statement", "")).strip()

            from src.agent.plan_agent_v1 import PlanAgentPipelineV1

            pipeline = PlanAgentPipelineV1(**pipeline_config)
            ...
            llm_summary = pipeline_result.total_metrics.get("llm_selection_summary", {})
            ...
            result = {
                ...
                "cheap_subtask_count": llm_summary.get("cheap_subtask_count", 0),
                "expensive_subtask_count": llm_summary.get("expensive_subtask_count", 0),
            }

            logger.info(
                f"[Worker-COAT-V1] Completed {instance_id}: resolved={result['resolved']}, "
                f"subtasks={result['subtask_count']}, "
                f"cheap={result['cheap_subtask_count']}, expensive={result['expensive_subtask_count']}"
            )
            return result

        except Exception as e:
            logger.error(f"[Worker-COAT-V1] Error in {instance_id}: {e}")
            traceback.print_exc()
            return {
                "instance_id": instance_id,
                "error": str(e),
            ...
            if workspace is not None:
                try:
                    ...
                    logger.warning(f"[Worker-COAT-V1] Cleanup failed for {instance_id}: {cleanup_err}")
...
def main():
    parser = argparse.ArgumentParser(
        description="并行运行SWE-bench评估 — COAT V1 (Dual-LLM PlanAgent) 模式"
    )
    ...
    parser.add_argument("--exp-config", type=str, required=True, help="Expensive 模型配置文件")
    parser.add_argument("--cheap-config", type=str, required=True, help="Cheap 模型配置文件")

    parser.add_argument(
        "--llm-selection-strategy",
        type=str,
        default="rule_based",
        choices=["rule_based", "llm_judged"],
        help="LLM 选择策略：rule_based（基于依赖数量）或 llm_judged（让模型自己判断）",
    )
    parser.add_argument(
        "--dependency-threshold",
        type=int,
        default=2,
        help="rule_based 策略下的依赖阈值，依赖数 >= 该值时使用 expensive LLM",
    )
    ...
    if args.output_dir is None:
        ...
        cheap_name = Path(args.cheap_config).stem
        timestamp = time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())
        args.output_dir = (
            f"./_tmp/coat_v1_{args.llm_selection_strategy}"
            f"_limit{args.eval_limit}"
            f"_{exp_name}_{cheap_name}"
            f"_{timestamp}"
        )
    ...
    run_config_record = {
        ...
        "llm_selection_strategy": args.llm_selection_strategy,
    ...
    logger.info(
        ...
        f"Strategy: {args.llm_selection_strategy}, "
        f"DependencyThreshold: {args.dependency_threshold}, "
    ...
    if args.parallel <= 1:
        for task in tasks:
            result = run_single_instance_plan_agent_v1(task)
        ...
        with ProcessPoolExecutor(max_workers=args.parallel) as executor:
            futures = {
                executor.submit(run_single_instance_plan_agent_v1, task): task
                for task in tasks
            }
    ...
    avg_cheap = sum(r.get("cheap_subtask_count", 0) for r in results) / max(total, 1)
    avg_expensive = sum(r.get("expensive_subtask_count", 0) for r in results) / max(total, 1)
    logger.info(f"Average subtasks per instance: {avg_subtasks:.1f} (cheap={avg_cheap:.1f}, expensive={avg_expensive:.1f})")
...
"""
# ============================================================
# COAT V1 — rule_based 策略 — doubao (expensive) + cheap model
# ============================================================
...
# ============================================================
# COAT V1 — llm_judged 策略 — doubao (expensive) + cheap model
# ============================================================
python example/benchmark_plan_agent_v1.py \
  --dataset ../_AutpPrep3_out/_data/SWEBenchVerified \
  --split test \
...
# ============================================================
# Quick test — single instance (rule_based)
# ============================================================
python example/benchmark_plan_agent_v1.py \
  --dataset ../_AutpPrep3_out/_data/SWEBenchVerified \
  --split test \
...
# ============================================================
# Quick test — single instance (llm_judged)
# ============================================================
python example/benchmark_plan_agent_v1.py \
  --dataset ../_AutpPrep3_out/_data/SWEBenchVerified \
  --split test \
...
# ============================================================
# COAT V1 — rule_based — glm5.1 (expensive) + cheap model
# ============================================================
python example/benchmark_plan_agent_v1.py \
  --dataset ../_AutpPrep3_out/_data/SWEBenchVerified \
  --split test \
